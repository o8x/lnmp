...
# 在 Linux 上使用 MySQL Yum 仓库安装 MySQL

Installing MySQL on Linux Using the MySQL Yum Repository: [https://dev.mysql.com/doc/refman/5.7/en/linux-installation-yum-repo.html](https://dev.mysql.com/doc/refman/5.7/en/linux-installation-yum-repo.html)
...
```mysql
mysql> alter user 'root'@'localhost' identified by 'Vt!B2Tp]4w3gCA>$';
...
```mysql
mysql> create user 'user'@'localhost' identified by 'MS:f1jp=2;9]Gs+g';
Query OK, 0 rows affected (0.00 sec)
```
...
```mysql
mysql> grant usage on user.* to 'user'@'%' identified by 'MS:f1jp=2;9]Gs+g' with grant option;
...
```mysql
mysql> grant all privileges on user.* to user@"%" identified by 'MS:f1jp=2;9]Gs+g';
Query OK, 0 rows affected, 1 warning (0.00 sec)
```
...
```mysql
mysql> grant all privileges on user.* to user@"1.1.1.1" identified by 'MS:f1jp=2;9]Gs+g';
Query OK, 0 rows affected, 1 warning (0.00 sec)
```
...
参数：-h MySQL Server 地址 -u 用户名 -p 使用密码进行连接

```shell
$ mysql -h remote.mysql-server.addr -u user -p
Enter password:
...
```mysql 
mysql> update user set host = "localhost" where user = "user" and host = "%";
Query OK, 0 rows affected (0.00 sec)
```
